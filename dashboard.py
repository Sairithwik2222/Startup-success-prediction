...
from utils import export_predictions_to_csv, export_predictions_to_json
...
def generate_sample_predictions_data(n_samples=500):
    ...
    for i in range(n_samples):
        ...
        funding_amount_map = {
            '0-100K': np.random.uniform(10000, 100000),
            '100K-500K': np.random.uniform(100000, 500000),
            '500K-1M': np.random.uniform(500000, 1000000),
            '1M-5M': np.random.uniform(1000000, 5000000),
            '5M+': np.random.uniform(5000000, 20000000)
        }
        funding_amount = funding_amount_map[funding_range]
...
def show_cohort_analysis(df):
    ...
    if 'funding_amount' in df_cohort.columns:
        agg_dict['funding_amount'] = 'mean'
        cohort_stats = df_cohort.groupby('month').agg(agg_dict).round(2)
        cohort_stats.columns = ['Avg Success %', 'Count', 'Avg Team Size', 'Avg Funding']
    else:
        cohort_stats = df_cohort.groupby('month').agg(agg_dict).round(2)
        cohort_stats.columns = ['Avg Success %', 'Count', 'Avg Team Size']
